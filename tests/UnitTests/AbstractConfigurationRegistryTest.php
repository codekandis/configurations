...
class AbstractConfigurationRegistryTest extends TestCase
{
	...
	/**
	 * Tests if the method `AbstractConfigurationRegistry::__clone()` throws a `CodeKandis\Configurations\CloningConfigurationRegistryIsProhibitedExceptionInterface`.
	 * @param string $configurationRegistryClassName The class name of the configuration to test.
	...
	public function testIfMethodCloneThrowsCloningConfigurationRegistryIsProhibitedExceptionInterface( string $configurationRegistryClassName, string $expectedThrowableClassName, string $expectedThrowableMessage ): void
	{
		try
		{
		...
		{
			static::assertInstanceOf( CloningConfigurationRegistryIsProhibitedExceptionInterface::class, $throwable );
